...
plot(SeqData_stock5$MaxMutFreq[which(SeqData_stock5$TotalReads>6000)],
     SeqData_stock6$MutFreqMaxMutStock[which(SeqData_stock5$TotalReads>6000)], 
     log="xy", col=alpha(cols,0.5), pch=16,
          main = "Comparison between two stock samples, R21", xlab = "Stock 5 Mutation frequencies",ylab = "Stock 6 Mutation frequencies"
     , xlim = c(0.00001,0.5), ylim = c(0.00001,0.5) )
abline(a = 0, b = 1)
...
plot(SeqData_stock5$MaxMutFreq[which(SeqData_stock5$TotalReads>6000)],SeqData_stock6$MutFreqMaxMutStock[which(SeqData_stock5$TotalReads>6000)], col=alpha(cols,0.7), pch=16,
     main = "Comparison between two stock samples, R21", xlab = "Stock 5 Mutation frequencies",ylab = "Stock 6 Mutation frequencies"
...
#June 2020
#find which sites have high freq in Stock5 but not Stock 6 (= machine control)

points(SeqData_stock5$MaxMutFreq[which(SeqData_stock5$MaxMutFreq>8*10^-3 & SeqData_stock5$TotalReads>6000)], 
       SeqData_stock6$MutFreqMaxMutStock[which(SeqData_stock5$MaxMutFreq>8*10^-3 & SeqData_stock5$TotalReads>6000)])
SeqData_stock5$pos[which(SeqData_stock5$MaxMutFreq>8*10^-3 & SeqData_stock5$TotalReads>6000)]

View(SeqData_stock5[which(SeqData_stock5$MaxMutFreq>10^-2),])
